          ...
          for (const [groupKey, groupRoutes] of Object.entries(pr)) {
            if (Array.isArray(groupRoutes)) {
              groupRoutes.forEach((r) =>
                routes.push({ type: `plugin:${groupKey}`, key: pluginName, method: r.method, path: r.path })
              );
              continue;
            }
            ...
            if (groupRoutes && typeof groupRoutes === 'object') {
              for (const val of Object.values(groupRoutes)) {
                ...
                if (Array.isArray(val)) {
                  ...
                  routes.push({ type: `plugin:${groupKey}`, key: pluginName, method: val.method, path: val.path });
                }
              }
            } else {
              // Unexpected shape: log it for inspection
              console.log(`[debug:routes] plugin ${pluginName} group ${groupKey} has unexpected shape`);
            }
      }
    }
